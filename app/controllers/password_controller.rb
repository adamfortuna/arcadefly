class PasswordController < ApplicationController
  ...
  def create    
    ...
    if @user = User.find_for_forget(email)
    ...
    elsif @profile = Profile.find_by_email(email, :include => :user)      
      ...
      flash[:error] = "We couldn't find a user with the email address <b>#{email}</b>. Do you want to try <a href=\"#{signup_url}?profile[email]=#{email}\">signing up</a>?"
    end  
