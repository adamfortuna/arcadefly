module GamesHelper
  ...
  def favorite_game_link(game, options = {})
    ...
    link_to_remote(image_tag("#{ASSET_HOST}/images/icons/controller_add.png", :size => "16x16", :border => 0) + text, 
  ...
  def unfavorite_game_link(game, options = {})
    ...
    text = options[:show_text] ? " " + options[:text] : ''
    link_to_remote(image_tag("#{ASSET_HOST}/images/icons/controller_delete.png", :size => "16x16", :border => 0) + text, 
